...
class LeaseholdExport implements FromArray, WithHeadings, WithStyles, ShouldAutoSize, WithTitle
{
    protected $data;
    protected $summary;
    ...
    public function array(): array
    {
        ...
        foreach ($this->data as $item) {
            $rows[] = [
                $item['asset_code'],
                $item['lease_term'],
                $item['improvement_cost'],
                $item['depreciation_method'],
                $item['remaining_lease_period'],
                $item['nbv'],
            ];
        }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->getStyle('C2:C' . $rowCount . ',F2:F' . $rowCount)
            ->getNumberFormat()
            ->setFormatCode('#,##0.00');
