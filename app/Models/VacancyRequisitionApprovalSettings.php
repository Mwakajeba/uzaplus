...
class VacancyRequisitionApprovalSettings extends Model
{
    use HasFactory;

    protected $table = 'vacancy_requisition_approval_settings';
    ...
    {
        $approvers = $this->getApproversForLevel($level);
        // Convert approvers to integers for comparison (JSON arrays may store as strings)
        $approverIds = array_map('intval', $approvers);
        return in_array((int)$userId, $approverIds);
    }
