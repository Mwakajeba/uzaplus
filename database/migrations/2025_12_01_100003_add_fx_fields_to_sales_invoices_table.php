...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::table('sales_invoices', function (Blueprint $table) {
            ...
            // Note: currency and exchange_rate already exist from previous migration
            // We're adding the additional FX tracking fields
            if (!Schema::hasColumn('sales_invoices', 'amount_fcy')) {
                $table->decimal('amount_fcy', 20, 2)->nullable()->after('exchange_rate')->comment('Foreign currency amount');
            }
            if (!Schema::hasColumn('sales_invoices', 'amount_lcy')) {
                $table->decimal('amount_lcy', 20, 2)->nullable()->after('amount_fcy')->comment('Local currency (functional currency) amount');
            }
            if (!Schema::hasColumn('sales_invoices', 'fx_gain_loss')) {
                $table->decimal('fx_gain_loss', 20, 2)->default(0)->after('amount_lcy')->comment('Realized FX gain/loss');
            }
            if (!Schema::hasColumn('sales_invoices', 'fx_rate_used')) {
