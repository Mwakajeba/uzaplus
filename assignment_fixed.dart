...
class _AssignmentsPageState extends State<AssignmentsPage> {
  ...
  bool _shouldShowAssignment(Map item) {
    ...
    if (_filter == 'All') {
      ...
      // Upcoming, due soon, or overdue are pending
      return true; // We'll filter by list membership
    } else if (_filter == 'Submitted') {
      // Submitted or marked are submitted
      return true; // We'll filter by list membership
    }
  ...
  List<Map> _getFilteredList(List<Map<String, dynamic>> list) {
  ...
  Widget build(BuildContext context) {
    ...
    // Filter all lists
    final filteredUpcoming = _filter == 'All' || _filter == 'Pending' 
        ? _getFilteredList(upcoming) 
        : <Map<String, dynamic>>[];
    final filteredDueSoon = _filter == 'All' || _filter == 'Pending'
        ? _getFilteredList(dueSoon)
        : <Map<String, dynamic>>[];
    final filteredSubmitted = _filter == 'All' || _filter == 'Submitted'
        ? _getFilteredList(submitted)
        : <Map<String, dynamic>>[];
    final filteredMarked = _filter == 'All' || _filter == 'Submitted'
        ? _getFilteredList(marked)
        : <Map<String, dynamic>>[];
    final filteredOverdue = _filter == 'All' || _filter == 'Pending'
        ? _getFilteredList(overdue)
        : <Map<String, dynamic>>[];
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Chip(
                        label: Text('Pending', style: const TextStyle(color: Colors.white)),
                        backgroundColor: Colors.orange.shade700,
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(item['dueIn'] ?? '', style: TextStyle(color: Colors.grey.shade700)),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 6),
                  Text(item['subject'] ?? '', style: TextStyle(color: Colors.grey.shade700)),
                  const SizedBox(height: 8),
                  Text(item['days'] ?? '', style: TextStyle(color: Colors.grey.shade600)),
                ],
              ),
            ),
            ElevatedButton(
