    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumbs-with-icons :links="[
                ...
                ['label' => 'Expense #' . $expense->reference_number, 'url' => route('hotel.expenses.show', $expense->hash_id), 'icon' => 'bx bx-show'],
                ['label' => 'Edit Expense', 'url' => '#', 'icon' => 'bx bx-edit']
            ]" />
            <h6 class="mb-0 text-uppercase">EDIT HOTEL EXPENSE</h6>
            <hr />
            <div class="row">
                <div class="col-12">
                            ...
                            <div class="d-flex align-items-center">
                                <div>
                                    <h5 class="mb-0 text-dark">
                                        <i class="bx bx-edit me-2"></i>Edit Hotel Expense
                                    </h5>
                                    <p class="mb-0 opacity-75">Update expense information</p>
                                </div>
                            </div>
                        </div>
                        <div class="card-body">
                            <form id="hotelExpenseForm" action="{{ route('hotel.expenses.update', $expense->hash_id) }}"
                                method="POST">
                                @csrf
                                @method('PUT')
                                ...
                                <div class="row mb-4">
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="date" class="form-control form-control-lg @error('expense_date') is-invalid @enderror"
                                                id="expense_date" name="expense_date" value="{{ old('expense_date', $expense->date->format('Y-m-d')) }}" required>
                                            @error('expense_date')
                                    ...
                                    <div class="col-lg-6">
                                        <div class="mb-3">
                                            ...
                                            <input type="text" class="form-control form-control-lg" id="total_amount" value="{{ number_format($expense->amount, 2) }}" disabled>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="row mb-4">
                                    <div class="col-12 mb-3">
                                        ...
                                        <textarea class="form-control form-control-lg @error('description') is-invalid @enderror"
                                            ...
                                            placeholder="Describe this expense...">{{ old('description', $expense->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                        ...
                                        <div class="card border-primary">
                                            ...
                                            <div class="card-body">
                                                <div id="lineItemsContainer">
                                                    @foreach($expense->paymentItems as $index => $item)
                                                    <div class="line-item-row mb-3 p-3 border rounded" data-index="{{ $index }}">
                                                        <div class="row g-2 align-items-end">
                                                            <div class="col-md-5">
                                                                <label class="form-label fw-bold">Account <span class="text-danger">*</span></label>
                                                                <select class="form-select select2-single" name="line_items[{{ $index }}][chart_account_id]" required>
                                                                    <option value="">-- Select Account --</option>
                                                                    @foreach($chartAccounts as $acc)
                                                                        <option value="{{ $acc->id }}" {{ old("line_items.{$index}.chart_account_id", $item->chart_account_id) == $acc->id ? 'selected' : '' }}>
                                                            ...
                                                            <div class="col-md-4">
                                                                ...
                                                                <input type="text" class="form-control" name="line_items[{{ $index }}][description]" 
                                                                       value="{{ old("line_items.{$index}.description", $item->description) }}" placeholder="Optional">
                                                            </div>
                                                            <div class="col-md-2">
                                                                <label class="form-label fw-bold">Amount <span class="text-danger">*</span></label>
                                                                <input type="text" class="form-control line-amount-input" 
                                                                       name="line_items[{{ $index }}][amount]" 
                                                                       value="{{ old("line_items.{$index}.amount", number_format($item->amount, 2)) }}" 
                                ...
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="d-flex justify-content-start">
                                            <a href="{{ route('hotel.expenses.show', $expense->hash_id) }}"
                                    ...
                                    <div class="col-lg-6">
                                        <div class="d-flex justify-content-end align-items-center">
                                            <button type="submit" class="btn btn-success" id="saveBtn">
                                                <i class="bx bx-save me-2"></i>Update Expense
                                            </button>
                                        </div>
                                    </div>
                                </div>
...
@push('scripts')
    <script nonce="{{ $cspNonce ?? '' }}">
        $(document).ready(function () {
            ...
            // Bind formatting to existing inputs
            amountInputs().each(function() {
                bindAmountFormatting(this);
            });
            ...
            // Line items
            let lineIdx = {{ $expense->paymentItems->count() }};
            function addLine(){
                lineIdx++;
                const html = `
