...
<head>
    ...
    <title>Bill Payment #{{ $payment->reference }}</title>
    <style>
        body {
...
<body>
    <div class="header">
        <div class="header-left">
            @if($payment->user->company->logo)
                <img src="{{ asset('storage/' . $payment->user->company->logo) }}" alt="Company Logo" class="logo">
            @endif
            <div class="company-info">
                <div class="company-name">{{ $payment->user->company->name ?? 'Company Name' }}</div>
                <div class="document-title">BILL PAYMENT</div>
            </div>
        </div>
    ...
    <div class="voucher-info">
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $payment->date->format('M d, Y') }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Status</span>
                <span class="info-value">
                    <span class="status-badge status-{{ $payment->approved ? 'approved' : 'pending' }}">
                        {{ $payment->approved ? 'Approved' : 'Pending' }}
                    </span>
                </span>
            </div>
            @if($payment->approved && $payment->approvedBy)
                <div class="info-row">
                    <span class="info-label">Approved By</span>
                    <span class="info-value">{{ $payment->approvedBy->name }}</span>
                </div>
            @endif
        </div>
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">{{ $payment->supplier->name ?? 'N/A' }}</span>
            </div>
            <div class="info-row">
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <span class="info-value">TZS {{ number_format($payment->amount, 2) }}</span>
            </div>
            <div class="info-row">
                <span class="info-label">Reference Type</span>
                <span class="info-value">{{ ucfirst($payment->reference_type) }}</span>
            </div>
            <div class="info-row">
    ...
    <div class="amount-section">
        ...
        <div class="amount-value">TZS {{ number_format($payment->amount, 2) }}</div>
    </div>

    @if($payment->paymentItems->count() > 0)
        <div class="line-items">
            <h3>Payment Line Items</h3>
            <table>
                <thead>
                    <tr>
                        <th style="width: 8%;">#</th>
                ...
                <tbody>
                    @foreach($payment->paymentItems as $index => $item)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td>
                                {{ $item->chartAccount->account_code ?? 'N/A' }} - 
                    ...
                    <tr class="total-row">
                        ...
                        <td style="text-align: right;"><strong>TZS {{ number_format($payment->amount, 2) }}</strong></td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    @if($payment->description)
        <div class="notes-section">
            ...
            <div class="notes-content">{{ $payment->description }}</div>
        </div>
    @endif
