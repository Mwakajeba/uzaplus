...
<body>
    <div class="header">
        <div class="header-content">
            ...
            <div class="title-section">
                <h1>PO vs GRN (Fulfillment) Report</h1>
                @if($company)
    ...
    <div class="report-info">
        ...
        <div class="info-grid">
            ...
            <div class="info-row">
                ...
                <div class="info-value">{{ ucfirst(str_replace('_', ' ', $fulfillmentStatus)) }}</div>
            </div>
            @endif
        </div>
    </div>
    ...
    @if($reportData->count() > 0)
        ...
        <div class="summary-stats">
            ...
            <div class="stat-item">
                <div class="stat-value">{{ $fullyReceived }}</div>
                <div class="stat-label">Fully Received</div>
            </div>
            <div class="stat-item">
                <div class="stat-value">{{ $partiallyReceived }}</div>
            ...
            <div class="stat-item">
                <div class="stat-value">{{ number_format($totalOrderedQty, 2) }}</div>
                <div class="stat-label">Total Ordered Qty</div>
            </div>
            <div class="stat-item">
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th>Item Code</th>
                    <th>Item Name</th>
                    <th class="number">Ordered Qty</th>
                    <th class="number">Received Qty</th>
                    <th class="number">Variance</th>
                    <th class="number">Fulfillment %</th>
                    <th>Status</th>
                </tr>
            </thead>
            <tbody>
                @foreach($reportData as $row)
                    <tr>
                        <td>{{ $row['po_number'] }}</td>
                        <td>{{ $row['po_date'] ? \Carbon\Carbon::parse($row['po_date'])->format('m/d/Y') : 'N/A' }}</td>
                        <td>{{ $row['supplier_name'] }}</td>
                        <td>{{ $row['branch_name'] }}</td>
                        <td>{{ $row['item_code'] }}</td>
                        <td>{{ $row['item_name'] }}</td>
                        <td class="number">{{ number_format($row['ordered_quantity'], 2) }}</td>
                        ...
                        <td class="number">{{ number_format($row['fulfillment_percentage'], 2) }}%</td>
                        <td>
            ...
            <tfoot>
                <tr style="background: #f8f9fa; font-weight: bold;">
                    ...
                    <td class="number">{{ number_format($totalOrderedQty, 2) }}</td>
                    ...
                    <td class="number">{{ $totalOrderedQty > 0 ? number_format(($totalReceivedQty / $totalOrderedQty) * 100, 2) : 0 }}%</td>
                    <td></td>
                </tr>
            </tfoot>
        </table>
