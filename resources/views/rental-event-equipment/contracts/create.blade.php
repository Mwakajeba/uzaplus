...
@section('title', 'Create Rental Contract')
...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">CREATE RENTAL CONTRACT</h6>
        <hr />

        @if($quotation)
        <div class="alert alert-info">
            <i class="bx bx-info-circle me-2"></i>
            <strong>Converting Quotation:</strong> {{ $quotation->quotation_number }} - Customer: {{ $quotation->customer->name }}
        </div>
        @endif

        <div class="card">
            <div class="card-header bg-success text-white">
                <h5 class="mb-0"><i class="bx bx-file me-2"></i>New Rental Contract</h5>
            </div>
            <div class="card-body">
                <form id="contract-form" method="POST" action="{{ route('rental-event-equipment.contracts.store') }}">
                    @csrf
                    @if($quotation)
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="event_date" name="event_date" 
                                       value="{{ $quotation ? $quotation->event_date?->format('Y-m-d') : '' }}">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="event_location" name="event_location" 
                                       value="{{ $quotation ? $quotation->event_location : '' }}" placeholder="Event location">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    </div>
                                        ...
                                        @if($quotation)
                                            ...
                                            <tr data-equipment-id="{{ $item->equipment_id }}" data-item-id="item_{{ $index }}">
                                                <td>
                                                    ...
                                                    <small class="text-muted">{{ $item->equipment->category->name ?? 'N/A' }}</small>
                                        ...
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                ...
                                                <select class="form-select form-select-sm d-inline-block ms-2" style="width: 120px;" id="discount_type" name="discount_type">
                                                    <option value="" {{ !$quotation || !$quotation->discount_type ? 'selected' : '' }}>None</option>
                                                    <option value="percentage" {{ $quotation && $quotation->discount_type === 'percentage' ? 'selected' : '' }}>Percentage</option>
                                                    <option value="fixed" {{ $quotation && $quotation->discount_type === 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                                                </select>
                                                <input type="number" class="form-control form-control-sm d-inline-block ms-2" style="width: 120px;" 
                                                       id="discount_amount" name="discount_amount" value="{{ $quotation ? $quotation->discount_amount : 0 }}" step="0.01" min="0" placeholder="0.00">
                                            </td>
                                            <td><strong id="discount-display">0.00</strong></td>
                                            <td></td>
                                        </tr>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="4" 
                                          placeholder="Additional notes...">{{ $quotation ? $quotation->notes : '' }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="terms_conditions" name="terms_conditions" rows="4" 
                                          placeholder="Terms and conditions...">{{ $quotation ? $quotation->terms_conditions : '' }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 justify-content-end mt-4">
                        <a href="{{ route('rental-event-equipment.contracts.index') }}" class="btn btn-outline-secondary">
                            <i class="bx bx-x me-1"></i>Cancel
                        </a>
                        <button type="submit" class="btn btn-success" id="submit-btn">
                            <i class="bx bx-check me-1"></i>Create Contract
                        </button>
                    </div>
                </form>
            </div>
...
<!-- Add Customer Modal (same as quotation) -->
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="customer_phone" required placeholder="255XXXXXXXXX">
                            <small class="text-muted">Phone number must begin with 255</small>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
...
$(document).ready(function() {
    let itemCounter = {{ $quotation ? $quotation->items->count() : 0 }};
    ...
    $('#add-customer-btn').on('click', function() {
        ...
        $('#customer_name, #customer_phone').val('');
    ...
    $('#save-customer-btn').on('click', function() {
        ...
        if (!phone) {
            ...
            $('#customer_phone').addClass('is-invalid').siblings('.invalid-feedback').text('Phone number must begin with 255');
            hasError = true;
        } else if (phone.length < 12) {
            $('#customer_phone').addClass('is-invalid').siblings('.invalid-feedback').text('Phone number must be 12 digits (255XXXXXXXXX)');
            hasError = true;
        }
        ...
        $.ajax({
            ...
            method: 'POST',
            data: { name, phone, status, _token: '{{ csrf_token() }}' },
    ...
    $('#add-equipment-to-table').click(function() {
        ...
        if ($(`tr[data-equipment-id="${equipmentId}"]`).length > 0) {
            Swal.fire('Warning', 'This equipment is already added to the contract.', 'warning');
            return;
        }
    ...
    // Initialize totals on page load if quotation data exists
    $(document).ready(function() {
        @if($quotation)
        // Calculate initial totals from quotation items
        calculateTotals();
        @endif
    });
    ...
    // Initialize calculations if items exist
    ...
    $('#contract-form').on('submit', function(e) {
        ...
        // Create new FormData from scratch to avoid conflicts
        const formData = new FormData();
        
        // Add all non-item form fields
        ...
        // Collect items and add with numeric indices
        const items = [];
        $('#equipment-tbody tr').each(function() {
        ...
        // Add items with numeric indices
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire('Success', 'Contract created successfully! Equipment status updated to Reserved.', 'success').then(() => {
                    window.location.href = '{{ route("rental-event-equipment.contracts.index") }}';
                });
            },
            error: function(xhr) {
                submitBtn.prop('disabled', false).html('<i class="bx bx-check me-1"></i>Create Contract');
                ...
                if (xhr.status === 422) {
                    ...
                    Swal.fire('Error', 'Failed to create contract. Please try again.', 'error');
                }
            }
        });
    });
