...
@section('title', 'Edit Rental Contract')
...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">EDIT RENTAL CONTRACT</h6>
        <hr />

        <div class="card">
            <div class="card-header bg-success text-white">
                <h5 class="mb-0"><i class="bx bx-file me-2"></i>Edit Rental Contract: {{ $contract->contract_number }}</h5>
            </div>
            <div class="card-body">
                <form id="contract-form" method="POST" action="{{ route('rental-event-equipment.contracts.update', $contract) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="contract_date" name="contract_date" 
                                       value="{{ $contract->contract_date->format('Y-m-d') }}" required>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="event_date" name="event_date" 
                                       value="{{ $contract->event_date ? $contract->event_date->format('Y-m-d') : '' }}">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="rental_start_date" name="rental_start_date"
                                       value="{{ $contract->rental_start_date ? $contract->rental_start_date->format('Y-m-d') : '' }}">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" id="rental_end_date" name="rental_end_date"
                                       value="{{ $contract->rental_end_date ? $contract->rental_end_date->format('Y-m-d') : '' }}">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="event_location" name="event_location" 
                                       value="{{ $contract->event_location }}" placeholder="Event location">
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class="table" id="equipment-table">
                                    ...
                                    <tfoot>
                                        <tr>
                                            ...
                                            <td><strong id="subtotal">{{ number_format($contract->subtotal, 2) }}</strong></td>
                                            <td></td>
                                        </tr>
                                        <tr>
                                            <td colspan="4" class="text-end">
                                                ...
                                                <select class="form-select form-select-sm d-inline-block ms-2" style="width: 120px;" id="discount_type" name="discount_type">
                                                    ...
                                                    <option value="percentage" {{ $contract->discount_type === 'percentage' ? 'selected' : '' }}>Percentage</option>
                                                    <option value="fixed" {{ $contract->discount_type === 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                                                </select>
                                                <input type="number" class="form-control form-control-sm d-inline-block ms-2" style="width: 120px;" 
                                                       id="discount_amount" name="discount_amount" value="{{ $contract->discount_amount }}" step="0.01" min="0" placeholder="0.00">
                                            </td>
                                            <td><strong id="discount-display">{{ number_format($contract->discount_amount, 2) }}</strong></td>
                                            <td></td>
                                        </tr>
                                        <tr class="table-success">
                                            <td colspan="4" class="text-end"><strong>Total Amount:</strong></td>
                                            <td><strong id="total-amount">{{ number_format($contract->total_amount, 2) }}</strong></td>
                                            <td></td>
                                        </tr>
                                    </tfoot>
                                </table>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="notes" name="notes" rows="4" 
                                          placeholder="Additional notes...">{{ $contract->notes }}</textarea>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="terms_conditions" name="terms_conditions" rows="4" 
                                          placeholder="Terms and conditions...">{{ $contract->terms_conditions }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="d-flex gap-2 justify-content-end mt-4">
                        ...
                        <button type="submit" class="btn btn-success" id="submit-btn">
                            <i class="bx bx-check me-1"></i>Update Contract
                        </button>
                    </div>
                </form>
            </div>
...
$(document).ready(function() {
    let itemCounter = {{ $contract->items->count() }};
    ...
    $('#contract-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                Swal.fire('Success', 'Contract updated successfully!', 'success').then(() => {
                    window.location.href = '{{ route("rental-event-equipment.contracts.index") }}';
                });
            },
            error: function(xhr) {
                submitBtn.prop('disabled', false).html('<i class="bx bx-check me-1"></i>Update Contract');
