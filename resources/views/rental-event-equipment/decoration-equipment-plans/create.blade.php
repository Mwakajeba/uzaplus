...
@section('title', 'New Decoration Equipment Plan')
...
@section('content')
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <h6 class="mb-0 text-uppercase">CREATE DECORATION EQUIPMENT PLAN</h6>
            <hr />

            <div class="card shadow-sm">
                <div class="card-header bg-info text-white">
                    <h5 class="mb-0"><i class="bx bx-list-check me-2"></i>Plan Equipment for Decoration Job</h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('rental-event-equipment.decoration-equipment-plans.store') }}">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select id="decoration_job_id" name="decoration_job_id"
                                        ...
                                        @foreach($jobs as $job)
                                            <option value="{{ $job->id }}" {{ old('decoration_job_id', optional($selectedJob)->id ?? null) == $job->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="notes" name="notes" rows="2"
                                      ...
                                      placeholder="Any planning notes, event theme details, setup instructions...">{{ old('notes') }}</textarea>
                            @error('notes')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="alert alert-info small">
                            ...
                            This plan is for internal coordination only. Stock levels will change when you
                            <strong>issue equipment</strong> against this job.
                        </div>

                        <div class="d-flex justify-content-between">
                            <a href="{{ route('rental-event-equipment.decoration-equipment-plans.index') }}" class="btn btn-secondary">
                                <i class="bx bx-x me-1"></i>Cancel
                            </a>
                            <button type="submit" class="btn btn-info">
                                <i class="bx bx-save me-1"></i>Save Plan
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        ...
        function buildPlanItemRow(index, existing) {
            ...
            return '' +
                ...
                '</select>' +
                '</td>' +
                '<td class="text-muted small equipment-code">' + (existing && existing.code ? existing.code : '-') + '</td>' +
                '<td class="text-end small equipment-available">' + (existing && typeof existing.available !== 'undefined' ? existing.available : 0) + '</td>' +
                '<td>' +
